...
let rec data =
  ...
  | Bytes (_, s) -> D_bytes (Bytes.to_string s) (* FIXME:  *)
  ...
  | Seq (_, l) -> D_list (List.map data l)

and inst =
  let open Micheline in
  function
  | Seq (_, l) -> List.fold_left (fun acc i -> I_seq (acc, inst i)) I_noop l
